...
//                break;
    ...
    {
        $uploads = Gallery::where('user_id', Auth::user()->id);
        if ($request->search != null) {
    ...
    public function destroy(Request $request,$id)
    {
        try{
            if(env('FILESYSTEM_DRIVER') == 's3'){
                Storage::disk('s3')->delete(Gallery::where('id', $id)->first()->file_name);
            }
            else{
                unlink(public_path().'/'.Gallery::where('id', $id)->first()->file_name);
            }
            Gallery::destroy($id);
    ...
    public function get_preview_files(Request $request){
        ...
        $files = Gallery::whereIn('id', $ids)->get();
        return $files;
    }
    ...
    {
        $project_attachment = Gallery::find($id);
        try{
    ...
    {
        $file = Gallery::findOrFail($request['id']);
